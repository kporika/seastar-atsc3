...
- `scripts/run_all_integration.sh` — runs the sink/LLS/STLTP scripts above plus admin + **M7** + LCT word‑0 + MMTP lab (**E**–**Q**) + RTCM (**12×96**)
...
**S**/**O**/**H** combos, ALC semantics, ROUTE bindings, and **MMTP** payload modes beyond the current **header-prefix lab** (word‑0 · **ts_psn** · **packet_counter** · chained **X** extensions · **signalling** §**9.3.4** 16b prefix · **aggregation** length+body pairs · **ISOBMFF** Figure 3 **64b** payload prefix when **payload_type**=**0** · optional **DU_header** (Fig. 4/5, **FT**=**2**) · **ISOBMFF** **A**=**1** **DU_length**+DU pairs) remain the
...
**Closes:** UDP/IPv4 builder (partial: C++ datagram builder + sinks) · ROUTE/LCT packetizer (partial: LCT header word-0 YAML + gw prepend) · MMTP packetizer (partial: **`mmtp_header_*`** + **`mmtp_desc`** + gw prepend lab + **`mmt_probe`** strip/expect, integration **E**–**Q**) · Raptor10/RaptorQ FEC.
