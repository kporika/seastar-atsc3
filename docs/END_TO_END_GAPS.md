...
- **`--prepend-lct-word0`** (**`--lct-codepoint`**), optionally **`--lct-include-tsi --lct-tsi`** and/or **`--lct-include-toi --lct-toi`** (both flags ⇒ RFC order **TSI** then **TOI**, **`header_length_words` = 3**, max **2035** inline user octets vs **2039**/ **2043** for smaller prefixes): RFC 5651 LCT lab prefix **inside** ALP; **`GET /config`** echoes **`lct_include_*`** / **`lct_tsi`** / **`lct_toi`**
...
- `lib/runtime/ipv4_udp.{hh,cc}` — **M8** IPv4 + UDP encapsulation with IPv4/UDP checksums (`encapsulate_ipv4_udp`); used by **`ipv4udp-file://`** sink in `gw/sink.cc` (append wire to file); **`udp://`** uses kernel UDP (no user-space IP header). **`protocol/lct_rfc5651_word0.yaml`** — RFC 5651 first LCT header word (codegen); gateway **`--prepend-lct-word0`** prefixes that word (**optional** **`--lct-include-tsi`**, **`--lct-include-toi`**, or **both**) inside ALP (lab stitch). **MMTP header (codegen):** **`mmtp_header_word0.yaml`**, **`mmtp_header_ts_psn.yaml`**, **`mmtp_header_counter32.yaml`**, **`mmtp_header_extension.yaml`** (one **X** TLV per decode; repeat for chains); **ISOBMFF payload:** **`mmtp_payload_isobmff_prefix.yaml`** (Figure 3 rows 1–2), **`mmtp_payload_isobmff_du_header_timed.yaml`** / **`mmtp_payload_isobmff_du_header_non_timed.yaml`** (Figures 4–5 when **FT**=**2**); **GFD payload header:** **`mmtp_payload_gfd_header.yaml`** (Figure 6, **payload_type** **0x01**); **DU_length** when **A**=**1**, opaque media bodies, signalling-mode payload header (**payload_type** **0x02**), MFU / PA / MPI / MPT modes, multi-extension **assembly**, **`gw`** MMTP prefix, full depacketizer not wired yet; ROUTE sessions / FEC not in **`atsc3_gw`** yet
...
| A/331 §10          | MMTP packetizer + signaling messages   | PARTIAL | **`mmtp_desc`** / **`mmtp_desc_loop`** (**Annex A.5** TLVs); **MMTP packet header (codegen):** **`mmtp_header_word0`**, **`mmtp_header_ts_psn`**, **`mmtp_header_counter32`**, **`mmtp_header_extension`** (type+length+opaque; one unit per YAML — repeat for **X** chains); **ISOBMFF payload:** **`mmtp_payload_isobmff_prefix`**, **`mmtp_payload_isobmff_du_header_timed`**, **`mmtp_payload_isobmff_du_header_non_timed`**; **GFD:** **`mmtp_payload_gfd_header`**; signalling payload header, **DU_length** / aggregation path, MFU / PA / MPI / MPT bodies, multi-extension assembly, **`gw`** MMTP prefix — still missing |
...
integration target. **Next transport slice:** MMTP **DU_length** when **A**=**1** (aggregation) + opaque **DU** bodies; **signalling** payload header (**payload_type** **0x02**); **gw** MMTP prefix (lab, like **LCT**).
