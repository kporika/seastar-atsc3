...
| A/331 §10          | MMTP packetizer + signaling messages   | PARTIAL | **`mmtp_desc`** / **`mmtp_desc_loop`** (**Annex A.5** TLVs); **MMTP packet header (codegen):** **`mmtp_header_word0`**, **`mmtp_header_ts_psn`**, **`mmtp_header_counter32`** (optional when **C**=**1**); **X** header extensions, MFU / PA / MPI / MPT payload headers + bodies, **`gw`** MMTP prefix — still missing |
...
integration target. **Next transport slice:** MMTP **header_extension** (**X**=**1**), payload-type-specific headers (MFU, PA/MPI/MPT), then optional **`gw`** lab prefix like **LCT**.
