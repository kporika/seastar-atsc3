...
const ROADMAP: ReadonlyArray<{
    ...
    },
    {
        ...
        blurb:
            ...
            "ALP encapsulation already accepts opaque payloads. Next transport slice: ISO §10.3 PA/MPI/MPT message headers and table bodies beyond the **`mmt_si_*`** descriptor-loop / length32 labs (and richer MFU beyond Figure 3 FT=1 header).",
                ...
                <Grid columns={2} gap={12}>
                    ...
                    <BulletGroup
                        ...
                        items={[
                            ...
                            "scripts/mmtp_word0_integration_test.sh — E/word‑0 only · F/word‑0+LCT · G/word‑0+ts_psn · H/word‑0+extension · I/word‑0+ts_psn+extension · J/word‑0+ts_psn+packet_counter · K/full prefix stack · L/word‑0+signalling prefix · M/word‑0+chained X · N/word‑0+signalling aggregation · O/word‑0+ISOBMFF prefix (A=0) · P/word‑0+ISOBMFF aggregation (A=1, DU_length+DU) · R/word‑0+ISOBMFF A=1 + DU_header per DU (mmt_probe strip, aggregate-hex = media only) · Q/word‑0+ISOBMFF DU_header (T=0) · S/word‑0+GFD (payload_type 1) · T/ALP pc/hm · U/ISOBMFF FT=1 (MFU) · Y/signalling + §10.2 mmt_si_length32_envelope · Z/signalling + §10.3 mmt_si_descriptor_loop_u32 · ZA/nested Y+Z · ZB/signalling + §10.3 mmt_si_message_header_len32 · ZC/signalling + §10.3 mmt_si_pa_table_headers (PA table index) · ZD/signalling + PA table body (table_length = ingress) · ZE/signalling + multi-table concatenated bodies (MPT+PLT) · ZF/signalling + mixed PA rows + SI tail after bodies · ZG/signalling + mmt_si_mpt_table (0x20) · ZH/signalling + mmt_si_plt_table (0x80) · ZI/MPT body prefix · ZJ/PLT body prefix · V–X/signalling (payload_type 2) §9.3.4 + packet_id 0/1/2 (MMT-SI sub-flow lab); mmt_probe verify --strip-mmtp-word0 / --strip-mmtp-ts-psn / --strip-mmtp-packet-counter / --strip-mmtp-extension / --strip-mmtp-signalling-prefix / --strip-mmtp-signalling-aggregate-count / --strip-mmtp-isobmff-prefix / --strip-mmtp-isobmff-du-header / --strip-mmtp-gfd-header / --strip-mmt-si-message-header-len32 / --strip-mmt-si-length32-envelope / --strip-mmt-si-descriptor-loop-u32 [--expect-mmtp-*] [--expect-mmtp-isobmff-du-*] [--expect-mmtp-isobmff-aggregate-hex] [--expect-mmtp-gfd-*] [--expect-mmt-si-message-byte-length] [--expect-mmt-si-body-byte-length] [--expect-mmt-si-descriptor-loop-length] [--expect-alp-payload-config] [--expect-alp-header-mode] + optional --strip-lct-word0",
