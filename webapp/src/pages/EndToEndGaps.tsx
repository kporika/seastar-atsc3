/*
 * Web rendering of docs/end_to_end_gaps.canvas.tsx. The data tables and
 * structural layout are kept verbatim with the canvas source so the two
 * artifacts stay in lockstep — bump one, bump the other.
 */
...
const STACK: ReadonlyArray<{
    ...
    {
        layer: "Transport",
        blurb: "ROUTE/LCT for DASH, MMTP for MMT (§10.3 validate lab through AEA), Raptor10/RaptorQ FEC — LCT word-0 + mmt_si_* + consumption message_id=0 lab landed",
        status: "in-flight",
    },
    {
...
const GAPS: ReadonlyArray<GapRow> = [
    ...
    },
    {
        ...
        status: "in-flight",
        notes:
            "mmtp_desc + mmtp_desc_loop = Annex A.5 TLVs; mmtp_header_word0 + ts_psn + counter32 + extension (ISO/IEC 23008-1; one extension TLV per YAML); isobmff_prefix + du_length16 (A=1) + du_header timed/non-timed (FT=2); gfd_header (type 0x01); mmtp_payload_signalling_prefix (type 0x02, §9.3.4 first 16b); gw --prepend-mmtp-word0 + optional --prepend-mmtp-ts-psn + optional --prepend-mmtp-packet-counter + optional --prepend-mmtp-extension (--mmtp-extension TYPE:HEX multitoken chain or legacy type/hex) + optional --prepend-mmtp-signalling-prefix (--mmtp-signalling-*; with --mmtp-signalling-aggregation repeat --mmtp-signalling-aggregate-hex for §9.3.4 length+body pairs) + optional --prepend-mmtp-isobmff-prefix (--mmtp-isobmff-*; --mmtp-isobmff-aggregate-hex when aggregation; optional --prepend-mmtp-isobmff-du-header + --mmtp-isobmff-du-* when FT=2; payload_type 0) + optional --prepend-mmtp-gfd-header (--mmtp-gfd-*, payload_type 1) + optional --prepend-mmt-si-descriptor-loop-u32 + optional --prepend-mmt-si-length32-envelope + optional --prepend-mmt-si-message-header-len32 (--mmtp-si-message-id, --mmtp-si-message-version); mmt_probe verify --strip-mmtp-word0 / --strip-mmtp-ts-psn / --strip-mmtp-packet-counter / --strip-mmtp-extension / --strip-mmtp-signalling-prefix / --strip-mmtp-signalling-aggregate-count / --strip-mmtp-isobmff-prefix / --strip-mmtp-isobmff-du-header / --strip-mmtp-gfd-header (ISOBMFF: A=0 or A=1; with --expect-mmtp-isobmff-aggregate-hex + strip, expects are media-only after each DU_header; GFD: --expect-mmtp-payload-type 1 + --expect-mmtp-gfd-*) / --strip-mmt-si-message-header-len32 / --strip-mmt-si-length32-envelope / --strip-mmt-si-descriptor-loop-u32 / --expect-mmtp-isobmff-du-* / --expect-mmtp-isobmff-aggregate-hex + scripts/mmtp_word0_integration_test.sh (E–AEA; §10.2/§10.3 validate lab through AEA incl. message_id=0 consumption AAU–AEA with PA-inline ACZ–ADO, ACR–ACY/ADP–ADW, ADX–AEA); optional gw --alp-payload-config / --alp-header-mode (A/330 §5.2 pc/hm); mmt_probe --expect-alp-payload-config / --expect-alp-header-mode; production PA/MPI/MPT emit + receiver depacketizer still open",
    },
    {
        layer: "Transport",
        spec: "RFC 5053 / 6330",
    ...
    },
    {
        ...
        status: "in-flight",
        notes:
            "lib/runtime/ipv4_udp.{hh,cc} encapsulate_ipv4_udp + checksums; ipv4udp-file:// sink; udp:// kernel path; protocol/lct_rfc5651_word0.yaml (LCT first word) + gw prepend (optional BE32 TSI / TOI(O=1), or both); MMTP header YAML: word0 + ts_psn + counter32 + extension (codegen); gw lab: word-0 + optional ts_psn + optional packet_counter + optional chained X extensions + optional signalling prefix (+ aggregation length+body) before optional LCT; full ROUTE/ALC/MMTP stack not in gw yet",
    },
    {
        layer: "Link",
        spec: "A/330 §5.2",
...
// -----------------------------------------------------------------------------
// Roadmap — pragmatic order to close the gaps.
// -----------------------------------------------------------------------------
const ROADMAP: ReadonlyArray<{
    ...
    },
    {
        ...
        blurb:
            ...
            "LCT starts with protocol/lct_rfc5651_word0.yaml (RFC 5651 first header word); atsc3_gw --prepend-lct-word0 inside ALP; optional BE32 TSI via --lct-include-tsi, optional O=1 TOI via --lct-include-toi, or both (TSI then TOI, hdr_len_words=3, max user 2035 octets lab); --prepend-mmtp-word0 adds MMTP packet header word-0 before optional LCT; optional --prepend-mmtp-ts-psn (--mmtp-timestamp, --mmtp-psn); optional --prepend-mmtp-packet-counter (--mmtp-packet-counter, requires ts_psn); optional --prepend-mmtp-extension (--mmtp-extension-type/--mmtp-extension-hex or --mmtp-extension TYPE:HEX multitoken chain); optional --prepend-mmtp-signalling-prefix (--mmtp-signalling-fragmentation, --mmtp-signalling-reserved, --mmtp-signalling-length-extension, --mmtp-signalling-aggregation, --mmtp-signalling-fragment-counter; with aggregation repeat --mmtp-signalling-aggregate-hex); optional --prepend-mmtp-isobmff-prefix (--mmtp-isobmff-*, --mmtp-isobmff-aggregate-hex when A=1; optional --prepend-mmtp-isobmff-du-header + --mmtp-isobmff-du-* when FT=2; payload_type 0; lab A=0 or A=1). " +
            "Full ROUTE/LCT sessions, Raptor10/RaptorQ FEC, production gw MMTP emit beyond the prepend/validate lab, and a receiver-style depacketizer remain. The gw/mmt_probe harness already covers scripts/mmtp_word0_integration_test.sh phases E–AEA (ISOBMFF A=0|1, DU_length, optional DU_header strip/expect, GFD peel, ALP pc/hm, signalling §9.3.4 V–X, §10.2/§10.3 Y–AEA incl. consumption messages AAU–AEA: PA/MPI/MPT/PLT inline ACZ–ADO, PA+MPT asset ACR–ACY/ADP–ADW, PA+MPT/PLT tail ADX–AEA) + full MPT asset id 0/1/2 × location + descriptors4 and full PLT delivery/package matrix). " +
            "ALP encapsulation already accepts opaque payloads. Still open: wiring validated §10.3 bodies into live PA/MPI/MPT emit paths in atsc3_gw (not only prepend/validate labs).",
        ...
        closes: [
            ...
            "MMTP packetizer (partial: mmtp_header_* + mmtp_desc + mmt_si_* §10.3 validate lab (MPT/PLT tables, assets, packages, delivery) through AEA + gw prepend + mmt_probe strip/expect for word-0, ts_psn, counter, extensions, signalling §9.3.4, ISOBMFF/GFD peels; integration E–AEA; production depacketizer still open)",
...
// Page
// -----------------------------------------------------------------------------
export default function EndToEndGaps() {
    ...
    return (
        <Stack gap={28}>
            ...
            <Stack gap={12}>
                ...
                <Text tone="secondary">
                    ATSC 3.0 ("NextGen TV") is the IP-native next-generation
                    digital terrestrial broadcast standard from the Advanced
                    Television Systems Committee, replacing the MPEG-TS pipeline
                    of ATSC 1.0. The stack is split across half a dozen specs:
                    an OFDM/LDPC physical layer (A/321 bootstrap + A/322 PHY),
                    a studio-to-transmitter scheduler and tunnel (A/324 STLTP),
                    a link layer that carries IP packets over the air ({" "}
                    <Text as="span" weight="semibold">
                        A/330 — ALP + TLV multiplex
                    </Text>
                    ), and service signaling plus content delivery (A/331
                    LLS/SLS, ROUTE/DASH, MMTP). This project takes{" "}
                    <Text as="span" weight="semibold">
                        A/330 as its reference scope
                    </Text>{" "}
                    and implements it as a YAML-driven C++ codec generator plus
                    a Seastar gateway. Below is the full inventory of every
                    protocol-level component between an operator's "broadcast
                    this" and the RF exciter, grouped by layer, with the
            ...
            <Grid columns={4} gap={16}>
                ...
                />
                <Stat
                    value={inScopeMissing}
                    label="PARTIAL + MISSING (in scope)"
                    tone="warning"
                />
                <Stat
                    value={externalGaps}
                    label="Out of scope (exciter / RF)"
                    tone="info"
                />
            ...
            <Stack gap={8}>
                ...
                <Text tone="secondary" size="small">
                    Rows with <Text as="span" weight="semibold">roadmap: true</Text> in the
                    table source are the subset this repository explicitly tracks for “all
                    link-layer lab gaps closed”; they are independent of the full
                    operator→RF inventory above.
                </Text>
            </Stack>
            <Grid columns={4} gap={16}>
                ...
                <Stat
                    value={roadmapOpen}
                    label="Roadmap open"
                    tone={roadmapOpen === 0 ? "success" : "warning"}
                />
                <Stat
                    value={`${pctRoadmap}%`}
                    label="DONE ÷ roadmap rows"
                    tone="success"
                />
            </Grid>
                ...
                <Grid columns={2} gap={12}>
                    <BulletGroup
                        ...
                        items={[
                            ...
                            "Optional HTTP admin (--admin-http): POST /ingest, GET /config, POST /config/sink + PATCH/PUT /config (mutating body is {\"sink_uri\"} only), GET/POST/DELETE /services, optional --prepend-lct-word0 (--lct-codepoint; optional --lct-include-tsi --lct-tsi and/or --lct-include-toi --lct-toi), optional --prepend-mmtp-word0 (--mmtp-payload-type, --mmtp-packet-id) with optional --prepend-mmtp-ts-psn (--mmtp-timestamp, --mmtp-psn), optional --prepend-mmtp-packet-counter (--mmtp-packet-counter, requires ts_psn), optional --prepend-mmtp-extension (--mmtp-extension-type, --mmtp-extension-hex), optional --prepend-mmtp-signalling-prefix (--mmtp-signalling-fragmentation, --mmtp-signalling-reserved, --mmtp-signalling-length-extension, --mmtp-signalling-aggregation, --mmtp-signalling-fragment-counter), optional --alp-payload-config / --alp-header-mode (ALP §5.2 pc/hm), optional --services-state-file JSON persistence, GET /healthz, /readyz, /metrics; Operator tab in webapp (npm run dev) via Vite proxy /__atsc3_admin; tools/atsc3ctl.py for shell automation",
                        ]}
                    />
                    <BulletGroup
                        title="What it produces on the wire"
                        items={[
                            "ALP packet: 16-bit base header + opaque payload (≤ 2047 B); optional pc/hm via --alp-payload-config / --alp-header-mode (lab: no segmentation TLVs)",
                    ...
                    <BulletGroup
                        ...
                        items={[
                            ...
                            "lib/runtime/ipv4_udp.{hh,cc} — M8 encapsulation + checksums; ipv4udp-file:// sink in gw/sink.cc; protocol/lct_rfc5651_word0.yaml (RFC 5651 LCT word 0 + optional BE32 --lct-include-tsi / --lct-include-toi / both); gw --prepend-mmtp-word0 (--mmtp-payload-type, --mmtp-packet-id) + optional --prepend-mmtp-ts-psn + optional --prepend-mmtp-packet-counter + optional --prepend-mmtp-extension + optional --prepend-mmtp-signalling-prefix (+ --mmtp-signalling-aggregate-hex when aggregating) + optional --prepend-mmtp-isobmff-prefix (+ --mmtp-isobmff-*) before optional LCT",
                            ...
                            "protocol/mmt_si_* — §10.2/§10.3 validate lab (length32 envelope, descriptor loop, message header, PA tables, MPT/PLT tables + body prefixes, full MPT asset id 0/1/2 × location + descriptors4, full PLT delivery + package matrix); integration ZK–AAT + consumption AAU–AEA (Docker make image-integ-mmtp-word0 PASS)",
                    ...
                    <BulletGroup
                        ...
                        items={[
                            ...
                            "scripts/run_all_integration.sh — fixed order: integration + udp + ipv4udp + stltp + lls + admin + M7 + lct_word0 (A–D) + mmtp_word0 (E–AEA) + rtcm 12×96 (= make integ-all)",
                            ...
                            "scripts/mmtp_word0_integration_test.sh — E–U prefix/ISOBMFF/GFD/ALP · V–X signalling §9.3.4 · Y–ZF §10.2/§10.3 + PA · ZG–AAT MPT/PLT matrix · AAU–AEA consumption (message_id=0; PA-inline ACZ–ADO, ACR–ACY, ADP–ADW, ADX–AEA); mmt_probe strip/expect (--strip-mmtp-*, --strip-mmt-si-*, --expect-mmtp-*, --expect-mmt-si-*, --expect-alp-*) + optional --strip-lct-word0; see docs/END_TO_END_GAPS.md",
                            "make docker-ctest / make image-integ-mmtp-word0 — Linux+Seastar build + integration in Docker (docs/DOCKER_TESTING.md)",
                            ...
                            ".github/workflows/ci.yml — Python lint + codegen + codec_smoke + webapp npm build (no Docker); C++/integration: make build && make integ* locally, or make docker-ctest / make image-integ-* (docs/DOCKER_TESTING.md); workflow_dispatch",
                        ]}
                    />
                </Grid>
            </Stack>
            ...
            <Stack gap={12}>
                ...
                <Text tone="secondary" size="small">
                    ...
                    flagged EXTERNAL belong to the exciter or transmitter plant
                    and are intentionally out of scope for atsc3_proto.
                </Text>
                <div className="stack-diagram">
                    {STACK.map((row, idx) => (
                        <div className="stack-diagram__row" key={row.layer}>
                            <div className="stack-diagram__idx">
                                {String(idx + 1).padStart(2, "0")}
                            </div>
                            <div className="stack-diagram__label">{row.layer}</div>
                            <div className="stack-diagram__blurb">
                                <Text tone="secondary">{row.blurb}</Text>
                            </div>
                            <div className="stack-diagram__pill">
            ...
            <Stack gap={12}>
                ...
                <Text tone="secondary" size="small">
                    Every concrete component with its ATSC / IETF spec citation
                    and current status. Row color matches the status pill.
                </Text>
                <Table
            ...
            <Stack gap={12}>
                ...
                <Text tone="secondary" size="small">
                    ...
                    work and unlocks a concrete capability you can demo
                    end-to-end.
                </Text>
                <Stack gap={12}>
                    {ROADMAP.map((m) => (
            ...
            <Stack gap={12}>
                ...
                <Callout tone="info" title="Where atsc3_proto stops">
                    <Stack gap={6}>
                        <Text>
                            atsc3_proto's job ends at the A/324 STLTP wire (M10).
                            The exciter consumes those packets and is responsible
                            for generating the bootstrap (A/321), the inner
                            BCH+LDPC FEC, the bit interleaver, the constellation
                            mapper (QPSK through 4096-QAM NUC), the time
                            interleaver, and the OFDM modulator itself (A/322).
                            The transmitter plant then handles up-conversion, the
                            power amplifier, the mask filter, and the antenna.
                        </Text>
                        <Text>
                            Reference exciters that consume A/324 STLTP today:
                            GatesAir Maxiva XTE, Sinclair RoVer, Enensys
                            MUX-3000, Harmonic Electra X3. Any of them can act as
                            the downstream peer once M10 lands.
                        </Text>
                    </Stack>
                </Callout>
            </Stack>
                    ...
                    <Card>
                        ...
                        <CardBody>
                            <Stack gap={8}>
                                ...
                                <Text>
                                    Doesn't pass conformance, but a real exciter
                                    on the bench will accept it and emit OFDM.
                                    Lets you exercise the whole chain on a single
                                    PLP, single MODCOD before investing in the
                                    full M10 surface.
                                </Text>
                            </Stack>
                        </CardBody>
