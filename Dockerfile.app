...
# Run the full ctest matrix (5 fixture round-trip executables + the
# encoder_pipeline unit test) and the python smoke. RUN_TESTS=0 skips both
...
# builder stage). Everything else Seastar pulls in (libseastar, c-ares
# wrapper, lz4, sctp, numa, openssl, crypto++, ...) is statically linked
# into atsc3_gw via libseastar.a.
#
...
# python smoke). The C++ build tree, lib/generated/, and tests/ are dropped.
...
# Default to a stdout sink so `docker run` produces visible output. Override
